...
async function registerUser() {
    ...
    db.run(`INSERT INTO users (first_name, last_name, username, password, email) VALUES (?, ?, ?, ?, ?)`, [answer.first_name, answer.last_name, answer.username, hashedPassword, answer.email], function (err) {
        if (err) {
...
async function loginUser() {
    ...
    db.get(`SELECT * FROM users WHERE username = ?`, [answer.username], async (err, row) => {
        if (err) {
            console.error('Error fetching user:', err.message);
        }
        else if (row) {
            const user = row; // Explicitly cast to User type
